class ShotsController < ApplicationController
  ...
  def index
    ...
    if params[:category]
      ...
      return @shots = Shot.where(shot_category_id: cat_id).where(is_public: true).all.includes(:user).order("created_at DESC").page(params[:page]).per_page(10)
    end
    @shots = Shot.where(is_public: true).all.includes(:user).page(params[:page]).per_page(10).order("created_at DESC")
  end
